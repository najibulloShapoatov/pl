...
@section('content')
    ...
    <div class="content-body">
        ...
        <div class="row ">
            ...
            <div class="col-12 col-lg-auto mb-20">
                <div class="page-heading">
                    <h3>@lang('lang.edit_videocourse')</h3>
                </div>
            </div>
            ...
            <div class="row mbn-15">
                <div class="row">
                    <div class="col-12 mb-15">
                        <input id="editing_course_title" type="text" class="form-control" placeholder="@lang('lang.name')" value="{!! $mycourse->title !!}">
                    </div>
                    <div class="col-12 mb-15">
                        <select id="videocourse_category" class="form-control std">
                            ...
                            @if(count($categories)>0)
                                @foreach($categories as $item)
                                    @if($item->id == $mycourse->category_id)
                    ...
                    <div class="col-12 mb-15">
                        <input id="editing_course_duration" readonly type="text" class="form-control" placeholder="@lang('lang.duration')" value="{!! timeConvert($mycourse->duration) !!}">
                    </div>
                    ...
                    <div class="col-12 mb-15">
                        <div class="video-course-image col-7 pt-20" >
                            <input
                                ...
                                id="course_cover"
                                data-default-file="{{ ($mycourse->image)?'/public/uploads/videocourse/' . $mycourse->id . '/' . $mycourse->image : '' }}"
                    ...
                    <div class="col-12 mb-15">
                        <textarea id="editing_course_descr"  class="form-control" placeholder="@lang('lang.description')">{!! $mycourse->description !!}</textarea>
                    </div>
                        ...
                        <input id="video_title" type="text" class="form-control" placeholder="@lang('lang.name_video')">
                            <div class="d-flex">
                                ...
                                <div class="col-5 pr-0">
                                    <button data-id="{{ $mycourse->id }}" id="add_new_video_on_course" class="button button-primary button-outline float-sm-right mt-15">@lang('lang.upload')</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div data-id="{{$mycourse->id}}" id="course_editing_page" class="col-sm-12 col-xs-12-col-md-12 ">
            <div class="row mb-15 mt-20">
                <div class="row">
                    @if(count($mycourse->galeries) > 0)
                        @foreach($mycourse->galeries as $item)
                            <div id="video_edit_{{$item->id}}" class="col-md-2 col-sm-3 col-xs-12 col-lg-2 mb-10">
                                <div class="v-course-item d-block">
                                    @if($item->image)
                                        <img class="mb-10" src="/public/uploads/videocourse/{{ $mycourse->id . '/' . $item->id . '/' . $item->image }}" alt="">
                                    @else
                                        <img class="mb-10" src="/public/web_assets/images/default-v-course.jpg" alt="">
                                    @endif
                                    <span data-id="{{ $item->id }}" id="delete_video" class="delete d-flex justify-content-center align-items-center">
        ...
        <div class="col-sm-12">
            <div class="wrap  float-right">
                <a href="{{ url('/delete-course/' . $mycourse->id) }}">
                    <span class="mt-25 pt-5 delete-community mr-25">@lang('lang.delete_videocourse')</span>
                </a>
                <button id="save_edited_course" class="button button-success s-btn">@lang('lang.save')</button>
            </div>
        </div>
